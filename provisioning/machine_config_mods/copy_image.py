...
def install_system(configuration):
    ...
    if current_install_part.startswith("/dev/mmcblk0"):
        ...
        print(
            f"Unknown current root device: {current_install_part}, skipping installation"
        )
        return
    ...
    if True:
        ...
        subprocess.run(
            f"head -c 5G {source_location} | pv -q -L 10M > {target_dev}",
            shell=True,
            check=True,
        )
    ...
    print(
        "Mounting target boot partition on "
        + mount_point
        + " to update rootfs uuid in armbianEnv.txt "
        + boot_part
    )
    out = subprocess.run(
    ...
    if not os.path.isfile(armbianenv_file):
        print(
            "No armbianEnv.txt file found on target boot partition, cannot update rootfs uuid"
        )
    ...
    if remove_overlay:
        ...
        for line in part_list:
            if "mirte_root" in line:
                tokens = line.split()
                mirte_root_part_num = tokens[0][
                    -1
                ]  # last character is partition number
                break
    ...
    if remove_settings:
        ...
        for line in part_list:
            if "MIRTE" in line:
                ...
                mirte_settings_part_num = tokens[0][
                    -1
                ]  # last character is partition number
                break
        ...
        if current_install == "sd":
            ...
            for line in part_list:
                if "armbi_root" in line:
                    tokens = line.split()
                    armbian_root_part_num = tokens[0][
                        -1
                    ]  # last character is partition number
                    break
            if armbian_root_part_num is None:
                print(
                    "No armbi_root partition found on SD card, cannot remove OS partitions"
                )
            else:
                ...
                subprocess.run(
                    f"dd if=/dev/zero of={source_dev}p{armbian_root_part_num} bs=4M status=progress count=100",
                    shell=True,
                    check=True,
                )
    ...
    print(
        "Installation complete, please reboot the system to boot from the new device."
    )
    # reboot system
    if reboot:
        ...
        print(
            "Shutting down system to allow manual reboot..."
        )  # when only install (without sd remove), do not reboot as it will keep looping.
...
def cleanup_sd(configuration):
    ...
    try:
        ...
        if settings_part_num is not None:
            print(
                f"Moving settings partition {settings_part_num} to follow overlay partition"
            )
            start_pos = "1MiB"
            if overlay_part_num is not None:
